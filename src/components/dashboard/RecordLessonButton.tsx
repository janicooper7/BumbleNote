...
// `hasLessons` counts drafts too: a trial lesson can only be a student's first.
...
export default function RecordLessonButton({
  ...
  // Only a brand-new student can have a trial lesson, so ticking it narrows the
  // list to them — and the option disappears once no such student is left.
  const trialEligible = useMemo(() => students.filter((s) => !s.hasLessons), [students]);
  const pool = trial ? trialEligible : students;

  const showSearch = pool.length > 3;
  ...
  function pick(s: PickStudent) {
    ...
    setQuery("");
    void start(s.id, trial && !s.hasLessons);
  }
    ...
    <>
      ...
      {open && typeof document !== "undefined" && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/30 p-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-2xl border border-line bg-surface p-10 shadow-soft-md">
            {status === "idle" && (
              <>
                <div className="mb-1 font-display text-2xl text-ink uppercase tracking-[.03em]">
                  Record a lesson
                </div>
                <p className="mb-5 text-base text-ink-soft">Who is this lesson with?</p>
                ...
                {!quota.allowed ? (
                  ...
                  <>
                    ...
                    {filtered.length === 0 ? (
                      ...
                      <div className="-mr-1 max-h-[22rem] space-y-1 overflow-y-auto pr-1">
                        {filtered.map((s) => (
                          <button
                            key={s.id}
                            onClick={() => pick(s)}
                            className="flex w-full items-center gap-4 rounded-xl border border-transparent px-4 py-3 text-left transition-colors hover:border-brand-line hover:bg-brand-soft/50"
                          >
                            <Avatar initial={s.initial} size={44} />
                ...
                {quota.allowed && trialEligible.length > 0 && (
                  <label className="mt-3 flex items-center gap-2 text-base font-semibold text-ink">
                    ...
                    />
                    This is a new student&apos;s trial lesson — help fill in their profile
                  </label>
                )}
